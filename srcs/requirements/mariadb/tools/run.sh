#!/bin/bash
  if [ ! -f "/var/lib/mysql/tc.log" ]; then
    ...
    mysql_install_db --basedir=/usr --datadir='/var/lib/mysql' --user=mysql --pid-file='/run/mysqld/mysqld.pid' --socket='/run/mysqld/mysqld.sock' --port=3306 --bind-address=* --skip-networking=0 > /dev/null 
